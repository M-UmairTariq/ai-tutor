...
// import {
//   Mic, Send, ChevronLeft, MessageCircle, Clock, BarChart2,
...
//       console.log(chatHistory, "RawHistory")
...
//       }));
//       console.log(formatted, "Formatted Hostory")
...
const ChatWindow: React.FC<ChatWindowProps> = ({ onShowFeedback, onTopicImage }) => {
  ...
  const startRecording = async () => {
    ...
    if (!isSocketConnected || sessionLimitReached || chatCompleted) {
      logger.error("Cannot start recording.", { isSocketConnected, sessionLimitReached, chatCompleted });
      return;
    }
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      ...
      logger.error("Microphone access denied or error starting recording.", err);
      toast.error("Microphone access denied.");
      cleanupRecording();
    }
  };
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!message.trim() || !isSocketConnected) {
      logger.error("Cannot send message.", { message: message.trim(), isSocketConnected });
      return;
    }
    setMessages(prev => [...prev, { id: crypto.randomUUID(), type: 'sent', messageType: 'text', text: message.trim() }]);
    sendPlaceholder();
  ...
  const handleStillThere = (isContinuing: boolean) => {
    setIsInactiveDialogOpen(false);
    logger.info(`User responded to inactivity dialog. Continuing: ${isContinuing}`);
    if (isContinuing) {
  ...
  useEffect(() => {
    ...
    if (audioMsg && audioRefs.current[audioMsg.id]) {
      ...
      audioRefs.current[audioMsg.id].play()
        ...
        .catch(e => logger.error("Autoplay was prevented by the browser.", e));
    }
  }, [messages]);
  ...
  const toggleAudio = (id: string) => {
    const audioElement = audioRefs.current[id];
    if (!audioElement) return;
    logger.info(`Toggling audio for message ID: ${id}. Current playing: ${playingAudio}`);
    if (playingAudio === id) {
      audioElement.pause();
      setPlayingAudio(null);
    } else {
